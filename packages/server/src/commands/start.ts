...
export default class Start extends Command {
    ...
    /**
     * @description Injects secrets from aws secret manager into env variables
     */
    async initSecretManager() {
        try {
            // Initialize secret manager
            SecretManager.getInstance()
        } catch (error) {
            logger.error('There was an error initializing secret manager...', error)
            processExitCode = EXIT_CODE.FAILED
            // @ts-ignore
            process.emit('SIGINT')
        }
