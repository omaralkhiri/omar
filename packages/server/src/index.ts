...
export class App {
    ...
    async config(socketIO?: Server) {
        ...
        // beat api
        this.app.get('/api/v1/beat', async (req: Request, res: Response) => {
            return res.status(200).send('OK')
        })

        // healthcheck api
        this.app.get('/api/v1/healthcheck', async (req: Request, res: Response) => {
            // execute a query to get postgres details
            const query = 'SELECT version();'
            const result = await this.AppDataSource.query(query)
        ...
        this.app.use('/flowiseai', express.static(uiBuildPath)) // TODO: prefix with flowiseai to set context path
